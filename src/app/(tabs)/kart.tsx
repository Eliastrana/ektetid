...
import { fetchLocatedPosts, regionFor, type LocatedPost } from '@/lib/map';
...
export default function MapScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setPosts(await fetchLocatedPosts());
    } catch {
  ...
  return (
    <View className="flex-1 bg-canvas">
      {region ? (
        ...
        <View className="flex-1 items-center justify-center gap-2 px-8">
          {loading ? (
            ...
            <>
              <Text className="text-base text-ink">Ingen øyeblikk på kartet</Text>
      ...
      <Screen className="absolute inset-0" edges={['top', 'bottom']} pointerEvents="box-none">
        <View className="px-5 pt-2" pointerEvents="none">
          <Text className="text-3xl text-ink">Kart</Text>
          {posts.length > 0 ? (
            <Text className="text-sm text-muted">
              {posts.length} {posts.length === 1 ? 'øyeblikk' : 'øyeblikk'}
            </Text>
          ) : null}
        </View>
